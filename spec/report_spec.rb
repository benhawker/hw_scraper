...
RSpec.describe Report do
  ...
  describe "#generate!" do
    it "raises an error if the city is not specified for the given partner" do
      ...
      cities = ["Moscow"]
      expect { described_class.new(partners, cities).generate! }.to raise_error(error_message)
    end

    it "raises an error if the city is not specified for the given partner" do
      error_message = 'One of your requested partners: ["bad_partner"] is not valid. Valid partners are: ["homeaway", "tujia"]'
      expect { described_class.new(["bad_partner"], cities).generate! }.to raise_error(error_message)
    end
  end
end