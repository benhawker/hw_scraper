...
RSpec.describe Comparer do
  ...
  let(:partner_props) { [{"title"=>"牛津街考文特花园完美伦敦公寓", "page"=>1}, {"title"=>"1号曼斯里大厦三室两卫公寓", "page"=>1}] }
  let(:rm_props) { [{:title=>"title_1", :rm_id=>"1", :city=>"london"}, {:title=>"title_2", :rm_id=>"2", :city=>"london"}] }

  describe "#compare" do
    before do
      allow_any_instance_of(described_class).to receive(:rm_props).and_return(rm_props)
      allow_any_instance_of(described_class).to receive(:partner_props).and_return(partner_props)
